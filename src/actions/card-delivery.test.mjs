...
async function fixture(t) {
  ...
  const options = {
    directory, afterId: 1310, term: '2026-fall', backendUrl, websiteToken: 'private-test-token',
    ...
    fetch: async (url, request) => {
      ...
      return { ok: true, json: async () => ({ ok: true, result: { status: 'saved', dashboardsUpdated: true } }) };
    },
  };
  return { options, calls, logs, rows, state: async () => JSON.parse(await readFile(join(directory, 'progress.json'), 'utf8')) };
}

test('delivers native rows only to the new database above the explicit cutoff, with stable fields and no PII checkpoint', async t => {
  const h = await fixture(t);
  await createCardDelivery(h.options).drain();
  assert.deepEqual(h.calls.map(x => [x.url, x.body.submissionID]), [
    [backendUrl, 1311], [backendUrl, 1312],
  ]);
  const v2 = h.calls[0].body;
  ...
  assert.equal(v2.additionalDept, 'PHYS');
  assert.equal(v2.teaching, false);
  await createCardDelivery(h.options).drain();
...
test('failed posts remain pending; term and ID survive restart', async t => {
  const h = await fixture(t);
  const fetch = h.options.fetch;
  h.options.fetch = async () => { throw new Error('private network failure'); };
  await createCardDelivery(h.options).drain();
  assert.equal(h.calls.length, 0);
  assert.deepEqual((await h.state()).pending, { submissionID: 1311, term: '2026-fall' });
  h.options.fetch = fetch;
  h.options.term = '2027-spring';
  await createCardDelivery(h.options).drain();
  assert.deepEqual(h.calls.map(x => [x.body.submissionID, x.body.term]), [[1311, '2026-fall'], [1312, '2027-spring']]);
  assert.equal((await h.state()).cursor, 1312);
});
...
test('quarantines rejected IDs without blocking later rows; explicit retry removes successful rejection metadata', async t => {
  ...
  h.options.fetch = async (url, request) => url === backendUrl && JSON.parse(request.body).submissionID === 1311
    ...
    : fetch(url, request);
  await createCardDelivery(h.options).drain();
  assert.equal((await h.state()).rejected['1311'].code, 'INVALID_REQUEST');
  assert.equal((await h.state()).cursor, 1312);
  h.options.fetch = fetch;
  await createCardDelivery(h.options).drain({ retryRejected: true });
  assert.deepEqual((await h.state()).rejected, {});
...
test('does not accept HTTP, auth, JSON or incomplete-refresh failures as delivered', async t => {
  for (const response of [
    ...
    const h = await fixture(t);
    h.options.fetch = async () => response;
    await createCardDelivery(h.options).drain();
    assert.equal((await h.state()).cursor, 1310);
    assert.deepEqual((await h.state()).rejected, {});
  }
});
