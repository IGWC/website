...
The union-card action still saves to `IGWCSubmissions` first. Saved rows are then POSTed independently to the legacy card sheet and the new Sheets database. The form and Turso schema are unchanged. This needs one persistent Node process, as used by the existing Docker service.
...
2. Copy `.env.card-delivery.example` into the private server file `../data/card-delivery.env`, preserve the existing backend URL/token, and supply the legacy Apps Script URL. Both URLs must be deployed `/exec` endpoints. Keep this file mode 600 and outside Git. Set the dedicated website token, not a dashboard access token.
...
7. GET `/card` as the readiness check after restart. Astro loads middleware on the first request, starting the poller. It also kicks after each new Turso insert and polls every minute, up to 50 new rows per destination each pass. Verify both cursors and affected dashboards for a real submission; do not submit fake production cards.

No database tables are dropped, cleared, or migrated by this integration. Disabling delivery stops forwarding while Turso continues collecting rows; re-enabling resumes from the saved progress.

## Delivery and recovery

`progress.json` stores per-destination cursors, the currently pending ID/term, and rejected IDs/terms. It contains no names, contact information, or tokens. Invalid or unreadable progress stops delivery rather than guessing a cursor. The original cutoff must remain unchanged when resuming an existing file.
...
Logs contain only destination, outcome, submission ID, and fixed error codes. Check both cursors against the latest native submission ID and inspect retained rejections; a successful form submission means Turso saved it, not that both Sheets writes have already finished. Never delete or reset progress just to retry a row.

The new receiver deduplicates by native submission ID. The legacy script always appends, so an acknowledgement lost after a successful write can produce a duplicate legacy row. This cannot be eliminated without changing that endpoint.
...
The legacy POST keeps the existing header names. Additional department/subfield and teaching are included as labeled text in `location`; its Apps Script still supplies its own arrival timestamp. The complete native row remains in Turso.
